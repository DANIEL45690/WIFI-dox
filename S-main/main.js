  ...
  return new Promise((resolve) => {
    ...
    if (platform === "win32") {
      exec("netsh wlan show interfaces", (err, stdout) => {
        ...
        const nameMatch =
          stdout.match(/名称\s*:\s(.+)/) || stdout.match(/Name\s*:\s(.+)/);
    ...
    return new Promise((resolve) => {
      exec("netsh wlan show interfaces", (err, stdout) => {
        ...
        resolve({
          ...
          bssid:
            (stdout.match(/BSSID\s*:\s(.+)/) || [])[1]?.trim() || "Unknown",
          signalStrength: parseInt(
            (stdout.match(/Signal\s*:\s(\d+)%/) || [])[1] || "0",
          ),
          receiveRate:
            (stdout.match(/接收速率\s*:\s(.+)/) ||
              stdout.match(/Receive rate\s*:\s(.+)/) ||
              [])[1]?.trim() || "0 Mbps",
          transmitRate:
            (stdout.match(/传输速率\s*:\s(.+)/) ||
              stdout.match(/Transmit rate\s*:\s(.+)/) ||
              [])[1]?.trim() || "0 Mbps",
          channel: (stdout.match(/Channel\s*:\s(\d+)/) || [])[1] || "Unknown",
          radioType:
            (stdout.match(/Radio type\s*:\s(.+)/) || [])[1]?.trim() ||
            "Unknown",
          authentication:
            (stdout.match(/Authentication\s*:\s(.+)/) || [])[1]?.trim() ||
            "Unknown",
          cipher:
            (stdout.match(/Cipher\s*:\s(.+)/) || [])[1]?.trim() || "Unknown",
          connected: true,
        });
      });
    });
    ...
    return new Promise((resolve) => {
      exec(
        ...
        (err, stdout) => {
          ...
          const rssi = parseInt(
            (stdout.match(/agrCtlRSSI:\s-?(\d+)/) || [])[1] || "0",
          );
          const noise = parseInt(
            (stdout.match(/agrCtlNoise:\s-?(\d+)/) || [])[1] || "0",
          );
          const signalStrength = Math.min(
            100,
            Math.max(0, Math.floor(100 + (rssi + noise) / 1.5)),
          );
          ...
          resolve({
            ...
            receiveRate: (stdout.match(/lastTxRate:\s(\d+)/) || [])[1]
              ? `${(stdout.match(/lastTxRate:\s(\d+)/) || [])[1]} Mbps`
              : "Unknown",
            transmitRate: (stdout.match(/lastTxRate:\s(\d+)/) || [])[1]
              ? `${(stdout.match(/lastTxRate:\s(\d+)/) || [])[1]} Mbps`
              : "Unknown",
    ...
    return new Promise((resolve) => {
      exec(
        ...
        (err, stdout) => {
          ...
          const signalStrength = signalMatch
            ? Math.min(
                100,
                Math.max(0, Math.floor(100 + parseInt(signalMatch[1]) / 2)),
              )
            : 0;
          ...
          resolve({
            ...
            bssid: "Unknown",
            signalStrength: signalStrength,
            receiveRate: (stdout.match(/Bit Rate=(\d+)/) || [])[1]
              ? `${(stdout.match(/Bit Rate=(\d+)/) || [])[1]} Mbps`
              : "Unknown",
            transmitRate: (stdout.match(/Bit Rate=(\d+)/) || [])[1]
              ? `${(stdout.match(/Bit Rate=(\d+)/) || [])[1]} Mbps`
              : "Unknown",
...
const getSignalQuality = (strength) => {
  ...
  if (strength >= 20)
    return { rating: "POOR", color: colors.red, bar: "██░░░░░░░░", icon: "⚠️" };
  return {
    rating: "VERY POOR",
    color: colors.red,
    bar: "█░░░░░░░░░",
...
const getChannelQuality = (channel, is5GHz) => {
  if (is5GHz) {
    ...
    if (dfsChannels.includes(channel))
      return {
        quality: "MEDIUM",
        note: "DFS channel - may have radar interference",
      };
    return { quality: "GOOD", note: "Clear channel" };
  } else {
    const overlapping = [1, 6, 11];
    if (overlapping.includes(channel))
      return { quality: "BEST", note: "Non-overlapping channel" };
  ...
  try {
    ...
    if (lastStats.upload > 0 && lastStats.timestamp) {
      ...
      const uploadSpeed = Math.max(
        0,
        (currentStats.upload - lastStats.upload) / timeDiff,
      );
      const downloadSpeed = Math.max(
        0,
        (currentStats.download - lastStats.download) / timeDiff,
      );
...
const refreshDisplay = async () => {
  try {
    ...
    if (!wifi.connected) {
      ...
      console.log(
        `${colors.red}║                          NO WIFI CONNECTION DETECTED                       ║${colors.reset}`,
      );
      console.log(
        `${colors.red}║                    Please connect to a WiFi network and retry              ║${colors.reset}`,
      );
      console.log(
        `${colors.red}╚════════════════════════════════════════════════════════════════════════════╝${colors.reset}`,
      );
    ...
    if (trafficHistory.length > 0) {
      ...
      avgUploadSpeed =
        recent.reduce((sum, t) => sum + t.uploadSpeed, 0) / recent.length;
      avgDownloadSpeed =
        recent.reduce((sum, t) => sum + t.downloadSpeed, 0) / recent.length;
    ...
    const is5GHz =
      wifi.radioType.includes("ac") ||
      wifi.radioType.includes("ax") ||
      channelNum > 36;
    ...
    asciiArt.forEach((line) => {
      ...
      console.log(
        `${colors.green}${" ".repeat(padding)}${line}${colors.reset}`,
      );
    });
    ...
    console.log(
      `${colors.bright}${colors.green}${" ".repeat(titlePadding)}${title}${colors.reset}`,
    );
    console.log();

    console.log(`${colors.green}╠${"═".repeat(width - 2)}╣${colors.reset}`);
    console.log(
      `${colors.bright}${colors.white} NETWORK INTERFACE ANALYSIS${colors.reset}`,
    );
    console.log(`${colors.gray}╠${"═".repeat(width - 2)}╣${colors.reset}`);

    console.log(
      `${colors.gray}║${colors.reset} ${colors.white}SSID:${colors.reset}              ${colors.green}${wifi.ssid.padEnd(35)}${colors.reset} ${colors.gray}│${colors.reset} ${colors.white}BSSID:${colors.reset}            ${colors.cyan}${wifi.bssid}${colors.reset}`,
    );
    console.log(
      `${colors.gray}║${colors.reset} ${colors.white}RADIO TYPE:${colors.reset}       ${colors.green}${(wifi.radioType || "802.11").padEnd(35)}${colors.reset} ${colors.gray}│${colors.reset} ${colors.white}CHANNEL:${colors.reset}           ${colors.cyan}${wifi.channel} (${is5GHz ? "5GHz" : "2.4GHz"})${colors.reset}`,
    );
    console.log(
      `${colors.gray}║${colors.reset} ${colors.white}RX RATE:${colors.reset}          ${colors.green}${(wifi.receiveRate || "N/A").padEnd(35)}${colors.reset} ${colors.gray}│${colors.reset} ${colors.white}TX RATE:${colors.reset}           ${colors.cyan}${wifi.transmitRate || "N/A"}${colors.reset}`,
    );

    console.log(`${colors.gray}╠${"═".repeat(width - 2)}╣${colors.reset}`);
    console.log(
      `${colors.bright}${colors.white} SIGNAL & PERFORMANCE${colors.reset}`,
    );
    console.log(`${colors.gray}╠${"═".repeat(width - 2)}╣${colors.reset}`);

    console.log(
      `${colors.gray}║${colors.reset} ${colors.white}SIGNAL:${colors.reset}           ${quality.color}${quality.bar} ${wifi.signalStrength}% (${quality.rating})${colors.reset}`,
    );
    console.log(
      `${colors.gray}║${colors.reset} ${colors.white}LATENCY:${colors.reset}          ${latency < 50 ? colors.green : latency < 150 ? colors.yellow : colors.red}${latency} ms${colors.reset} ${colors.gray}│${colors.reset} ${colors.white}PACKET LOSS:${colors.reset}     ${packetLoss < 1 ? colors.green : packetLoss < 5 ? colors.yellow : colors.red}${packetLoss.toFixed(2)}%${colors.reset}`,
    );
    console.log(
      `${colors.gray}║${colors.reset} ${colors.white}BANDWIDTH SCORE:${colors.reset}  ${bandwidthScore > 80 ? colors.green : bandwidthScore > 50 ? colors.yellow : colors.red}${bandwidthScore}/100${colors.reset} ${colors.gray}│${colors.reset} ${colors.white}CHANNEL QUALITY:${colors.reset}   ${channelAnalysis.quality == "BEST" ? colors.green : channelAnalysis.quality == "GOOD" ? colors.cyan : colors.yellow}${channelAnalysis.quality}${colors.reset}`,
    );

    console.log(`${colors.gray}╠${"═".repeat(width - 2)}╣${colors.reset}`);
    console.log(
      `${colors.bright}${colors.white} REAL-TIME TRAFFIC METRICS${colors.reset}`,
    );
    console.log(`${colors.gray}╠${"═".repeat(width - 2)}╣${colors.reset}`);
    ...
    const downloadPercent = Math.min(
      100,
      Math.floor((avgDownloadSpeed / (10 * 1024 * 1024)) * 100),
    );
    const uploadPercent = Math.min(
      100,
      Math.floor((avgUploadSpeed / (5 * 1024 * 1024)) * 100),
    );
    const downloadBars = "█".repeat(
      Math.floor((barWidth * downloadPercent) / 100),
    );
    const uploadBars = "█".repeat(Math.floor((barWidth * uploadPercent) / 100));

    console.log(
      `${colors.gray}║${colors.reset} ${colors.green}⬇ DOWNLOAD${colors.reset}  ${formatSpeed(avgDownloadSpeed).padEnd(12)} ${colors.green}[${downloadBars}${"░".repeat(barWidth - downloadBars.length)}]${colors.reset}`,
    );
    console.log(
      `${colors.gray}║${colors.reset} ${colors.cyan}⬆ UPLOAD${colors.reset}    ${formatSpeed(avgUploadSpeed).padEnd(12)} ${colors.cyan}[${uploadBars}${"░".repeat(barWidth - uploadBars.length)}]${colors.reset}`,
    );
    console.log(`${colors.gray}║${colors.reset}`);
    console.log(
      `${colors.gray}║${colors.reset} ${colors.white}PEAK DOWNLOAD:${colors.reset}  ${colors.green}${formatSpeed(peakDownload).padEnd(20)}${colors.reset} ${colors.white}PEAK UPLOAD:${colors.reset}    ${colors.cyan}${formatSpeed(peakUpload)}${colors.reset}`,
    );
    console.log(
      `${colors.gray}║${colors.reset} ${colors.white}TOTAL DOWNLOAD:${colors.reset} ${colors.green}${formatBytes(totalDownload).padEnd(20)}${colors.reset} ${colors.white}TOTAL UPLOAD:${colors.reset}   ${colors.cyan}${formatBytes(totalUpload)}${colors.reset}`,
    );

    console.log(`${colors.gray}╠${"═".repeat(width - 2)}╣${colors.reset}`);
    console.log(
      `${colors.bright}${colors.white} SECURITY POSTURE${colors.reset}`,
    );
    console.log(`${colors.gray}╠${"═".repeat(width - 2)}╣${colors.reset}`);
    ...
    const securityColor =
      security.score >= 80
        ...
        : security.score >= 60
          ...
          : colors.red;
    console.log(
      `${colors.gray}║${colors.reset} ${colors.white}AUTHENTICATION:${colors.reset}  ${securityColor}${security.auth || "Unknown"}${colors.reset}`,
    );
    console.log(
      `${colors.gray}║${colors.reset} ${colors.white}CIPHER:${colors.reset}          ${securityColor}${security.cipher || "Unknown"}${colors.reset}`,
    );
    console.log(
      `${colors.gray}║${colors.reset} ${colors.white}SECURITY SCORE:${colors.reset}  ${securityColor}${security.score}/100 (${security.level})${colors.reset}`,
    );

    console.log(
      `${colors.gray}║${colors.reset} ${colors.white}VULNERABILITIES:${colors.reset}`,
    );
    security.vulnerabilities.forEach((vuln) => {
      console.log(
        `${colors.gray}║${colors.reset}   ${colors.red}⚠${colors.reset} ${vuln}`,
      );
    });

    console.log(`${colors.gray}╠${"═".repeat(width - 2)}╣${colors.reset}`);
    console.log(
      `${colors.bright}${colors.white} OPTIMIZATION RECOMMENDATIONS${colors.reset}`,
    );
    console.log(`${colors.gray}╠${"═".repeat(width - 2)}╣${colors.reset}`);

    const recommendations = [];

    if (wifi.signalStrength < 50)
      recommendations.push(
        `${colors.yellow}• Move closer to the access point or remove obstacles${colors.reset}`,
      );
    if (latency > 100)
      recommendations.push(
        `${colors.yellow}• High latency detected - check for network congestion${colors.reset}`,
      );
    if (packetLoss > 2)
      recommendations.push(
        `${colors.red}• Packet loss detected - possible interference or distance issues${colors.reset}`,
      );
    if (security.score < 70)
      recommendations.push(
        `${colors.red}• Upgrade security configuration immediately${colors.reset}`,
      );
    if (channelAnalysis.quality === "POOR")
      recommendations.push(
        `${colors.yellow}• Change to a non-overlapping channel (1, 6, or 11 for 2.4GHz)${colors.reset}`,
      );
    if (bandwidthScore < 50)
      recommendations.push(
        `${colors.yellow}• Bandwidth is limited - check for signal interference${colors.reset}`,
      );
    if (avgDownloadSpeed > totalDownload * 0.1 && totalDownload > 0)
      recommendations.push(
        `${colors.cyan}• High background traffic detected - check running applications${colors.reset}`,
      );

    if (recommendations.length === 0) {
      recommendations.push(
        `${colors.green}✓ All metrics are optimal - network performance is excellent${colors.reset}`,
      );
    }
    ...
    const statusPadding = Math.max(
      0,
      Math.floor((width - statusText.length) / 2),
    );
    console.log(
      `${colors.green}${" ".repeat(statusPadding)}${statusText}${colors.reset}`,
    );
...
const init = async () => {
  ...
  if (!wifi.connected) {
    console.log(
      `${colors.red}No WiFi connection detected. Please connect to a network and restart.${colors.reset}`,
    );
    process.exit(1);
  }
  ...
  console.log(
    `${colors.cyan}Initializing real-time monitoring...${colors.reset}`,
  );
